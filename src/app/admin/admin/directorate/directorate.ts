...
import { DirectorateService } from '../../services/directorate';
...
@Component({
  ...
  templateUrl: './directorate.html',
  styleUrls: ['./directorate.css']
})
export class Directorate implements OnInit {
  items: any[] = [];
  searchTerm = '';
  ...
  private service = inject(DirectorateService);
  ...
  loadItems() {
    this.service.getDirectorates().subscribe({
      next: (data: any) => {
  ...
  onSubmit() {
    ...
    if (this.isEditing && this.editingId !== null) {
      this.service.updateDirectorate(this.editingId, formData).subscribe({
        next: () => {
          this.loading = false;
          this.message = 'Directorate member updated successfully!';
          this.resetForm();
          this.loadItems();
        },
        error: (err) => {
      ...
      this.service.createDirectorate(formData).subscribe({
        next: () => {
          this.loading = false;
  ...
  deleteItem(id: number) {
    this.confirmService.confirm('Are you sure you want to delete this directorate?').then((confirmed) => {
      if (confirmed) {
        this.service.deleteDirectorate(id).subscribe({
          next: () => this.loadItems(),
