...
export class Policy implements OnInit {
  ...
  ngOnInit(): void {
    // Fetch documents from backend API using doctype=3
    this.http.get<any>(`${environment.apiUrl}/documents?doctype=3`).subscribe({
      next: (response) => {
      ...
      error: (err) => {
        ...
        // this.error.set('Failed to load policy documents.');
        // Do not set isLoading to false; keep skeleton visible if no data/error
      }
    });
  }
  
  getPDFUrl(doc: PolicyDoc): string {
    if (doc.file_url) {
      return doc.file_url;
    }
